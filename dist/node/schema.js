...
var Schema = (function (_super) {
    ...
    Schema.cloneOptions = function (source) {
        var result = lodash_1.assign({}, source);
        if (source.properties) {
            if (!result.properties) {
                result.properties = {};
            }
            for (var propertyName in source.properties) {
                result.properties[propertyName] = lodash_1.assign({}, result.properties[propertyName], source.properties[propertyName]);
            }
        }
        return result;
    };
    Schema.assignOptions = function (target, source) {
        ...
        if (source.properties) {
            ...
            for (var propertyName in source.properties) {
                target.properties[propertyName] = lodash_1.assign({}, target.properties[propertyName], source.properties[propertyName]);
            }
        }
        return target;
    };
